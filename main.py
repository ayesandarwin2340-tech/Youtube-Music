...
COOKIES_FILE = "cookies.txt"  # Cookies file name
...
📌 **အသုံးပြုနည်း:**
• သီချင်းနာမည် သို့မဟုတ် link တိုက်ရိုက်ပို့ပါ
...
💡 **Inline Mode:**
ဘယ် chat မှာမဆို `@music5734_bot song name` လို့ရိုက်ပြီး ရှာနိုင်တယ်
...
📌 **How to use:**
• Send song name or link directly
...
💡 **Inline Mode:**
Type `@music5734_bot song name` in any chat to search
...
def get_cookies_path():
    ...
    cookies_path = os.path.join(bot_folder, COOKIES_FILE)
...
class DownloadQueue:
    ...
    async def add(self, user_id, task):
        async with self.lock:
            if len(self.user_queues[user_id]) >= 2:
...
class RateLimiter:
    ...
    def check_daily_limit(self, user_id: int) -> Tuple[bool, int]:
        ...
        c.execute('''SELECT COUNT(*) FROM downloads 
                     WHERE user_id = ? AND DATE(download_date) = DATE(?)''', (user_id, today))
        count = c.fetchone()[0]
        conn.close()
    ...
    def add_download(self, user_id: int, song_title: str, file_size: float):
        ...
        c.execute('''INSERT INTO downloads (user_id, song_title, file_size, download_date)
                     VALUES (?, ?, ?, ?)''', (user_id, song_title, file_size, datetime.now()))
        conn.commit()
        conn.close()
...
class CacheManager:
        ...
        if result:
            ...
            if datetime.now() - last_time < timedelta(seconds=CACHE_EXPIRY):
                if os.path.exists(file_path):
                    ...
                    conn.commit()
                    conn.close()
                    return {"file_path": file_path, "title": title, "artist": artist, "duration": duration}
        conn.close()
        return None
    ...
    def clean_cache():
        ...
        c.execute('''SELECT file_path FROM cache 
                     WHERE julianday('now') - julianday(last_accessed) > ?''', (CACHE_EXPIRY / 86400,))
...
# Combined /stats Command (User + Admin)
...
async def stats_command(client, message):
    ...
    if user.id == OWNER_ID:
        ...
        top_users_text = ""
        for uid, downloads in top_users:
            try:
                u = await client.get_users(uid)
                name = u.first_name or "Unknown"
                top_users_text += f"• {name}: {downloads} downloads\n"
            except:
                top_users_text += f"• User {uid}: {downloads} downloads\n"
        
        cookies_path = get_cookies_path()
        cookies_status = "✅ Active" if cookies_path else "❌ Not found"
...
{top_users_text}
        ...
        today = datetime.now().date()
        c.execute('''SELECT COUNT(*) FROM downloads 
                     WHERE user_id = ? AND DATE(download_date) = DATE(?)''', (user.id, today))
        today_count = c.fetchone()[0]
        
        c.execute('''SELECT song_title, COUNT(*) as cnt FROM downloads 
                     WHERE user_id = ? GROUP BY song_title ORDER BY cnt DESC LIMIT 1''', (user.id,))
        most = c.fetchone()
...
async def process_download(message, query):
    user = message.from_user
    lang = get_user_language(user.id)
    within_limit, today_count = rate_limiter.check_daily_limit(user.id)
    if not within_limit:
        await message.reply(LANG[lang]["daily_limit"].format(MAX_DOWNLOADS_PER_DAY=MAX_DOWNLOADS_PER_DAY))
        return
    ...
    if not added:
        ...
        return
    await message.reply(f"⏳ **Added to queue. Position: {queue_pos}**")
    asyncio.create_task(process_queue())

@app.on_message(filters.command("play"))
async def play_command(client, message):
    if not rate_limiter.check_flood(message.from_user.id):
    ...
    await process_download(message, query)

@app.on_message(filters.text & filters.private & ~filters.command(["start", "help", "settings", "stats", "language", "quality", "play", "broadcast", "cleanup", "cookies"]))
...
def extract_ydl_info(ydl, query, is_url):
    if is_url:
...
async def download_song(message, query):
    ...
    if cached:
        ...
        rate_limiter.add_download(user.id, cached["title"], os.path.getsize(cached["file_path"]) / (1024 * 1024))
    ...
    ydl_opts = {
        ...
        "retries": 10,
        "fragment_retries": 10,
        "extractor_args": {"youtube": {"skip": ["dash", "hls", "translated_subs"], "player_client": ["android", "web"]}},
    }
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "Requested format" in error_str or "format" in error_str.lower():
                        ...
                        try:
                            await status_msg.edit(f"⚠️ **Retry {attempt + 1}/{max_retries} in {wait_time}s...**")
                        except MessageNotModified: pass
            ...
            thumb_path = None
            if thumbnail:
                thumb_path = os.path.join(WORKING_DIR, f"thumb_{user.id}_{int(time.time())}.jpg")
                success = await asyncio.to_thread(download_image, thumbnail, thumb_path)
                if not success:
                    thumb_path = None
        ...
        if "Sign in to confirm" in error_msg:
            try:
                await status_msg.edit("❌ **YouTube Bot Detection** ❌\n🔒 YouTube က Bot လို့သတ်မှတ်လိုက်တယ်။\n🍪 Cookies ဖိုင်ကို update လုပ်ဖို့လိုတယ်။")
            except MessageNotModified: pass
        else:
            try:
                await status_msg.edit(f"❌ **Download Error:** {error_msg}")
            except MessageNotModified: pass
    except Exception as e:
        try:
...
async def inline_query(client, inline_query):
    ...
    ydl_opts = {"format": "bestaudio", "default_search": "ytsearch5", "quiet": True, "no_warnings": True, "extract_flat": True}
    try:
        def search():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        for entry in info["entries"]:
            ...
            results.append(
                InlineQueryResultArticle(
                    ...
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎵 Download", callback_data=f"dl_{video_id}")]])
                )
            )
...
async def handle_callback(client, callback_query):
    ...
    if data == "change_lang":
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🇲🇲 မြန်မာ", callback_data="lang_my"), InlineKeyboardButton("🇬🇧 English", callback_data="lang_en")],
            [InlineKeyboardButton("🔙 Back", callback_data="settings")]
        ])
        await callback_query.message.edit_text("Choose language:", reply_markup=keyboard)
    
    elif data in ["lang_my", "lang_en"]:
        lang = data.split("_")[1]
        update_user_language(user.id, lang)
        await callback_query.answer(f"Language set to {'မြန်မာ' if lang == 'my' else 'English'}")
        await settings_command(client, callback_query.message)
    
    elif data == "change_quality":
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("128 kbps", callback_data="quality_128"), InlineKeyboardButton("192 kbps", callback_data="quality_192")],
            [InlineKeyboardButton("320 kbps", callback_data="quality_320"), InlineKeyboardButton("🔙 Back", callback_data="settings")]
        ])
        ...
        callback_query.message.from_user = user
        ...
        video_id = data[3:]
        await callback_query.message.reply(video_id)
...
async def cookies_command(client, message):
    ...
    if cookies_path:
        ...
        mod_date = datetime.fromtimestamp(os.path.getmtime(cookies_path)).strftime("%Y-%m-%d %H:%M:%S")
        with open(cookies_path, 'r') as f:
            line_count = len(f.readlines())
        text = f"🍪 **Cookies Status**\n📁 Path: {cookies_path}\n📏 Size: {file_size} bytes\n📅 Mod: {mod_date}\n📝 Lines: {line_count}\n✅ Active"
    else:
        text = "❌ **Cookies File Not Found!**"
